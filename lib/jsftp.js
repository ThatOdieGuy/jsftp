 ...
 * @param {Number} skipBytes If set, will attempt to restart a previous download by skipping these byte.
 * @param {Function} callback Function to call when finalized, with the socket
 * as a parameter
 */
Ftp.prototype.getGetSocket = function(path, skipBytes, callback) {
  //Some function overloading to allow getGetSocket(path, callback)
  if (typeof skipBytes === 'function') {
    callback = skipBytes;
    skipBytes = 0;
  }
  ...
  this.getPasvSocket((err, socket) => {
    ...
    if (skipBytes === 0) {
      ...
      this.execute('rest ' + skipBytes, (err, res) => {
        if (err) {
          // Possibly only a warning if the server does not support resume?
          return cmdCallback(err);
        }
        this.execute('retr ' + path, cmdCallback);
      });
